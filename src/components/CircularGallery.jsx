...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    const rawScale = this.screen.height / 1500
    this.scale = Math.max(0.35, Math.min(rawScale, 1))
...
class App {
  ...
  onIntersection(entries) {
    const [entry] = entries
    this.isVisible = Boolean(entry?.isIntersecting) && !document.hidden
  }
  ...
  addEventListeners() {
    ...
    this.container.addEventListener('pointerdown', this.onTouchDown, { passive: true })
    window.addEventListener('pointermove', this.onTouchMove, { passive: true })
    window.addEventListener('pointerup', this.onTouchUp, { passive: true })
    window.addEventListener('pointercancel', this.onTouchUp, { passive: true })
